...
Input files must be either single PageXML files or zip archives of PageXML files. Input files must be in *date/input* folder and should contain the jaffe page number in its names (which is standard for eScriptorium PageXML export).
Possible output formats are tsv, csv, excel, xml (either as one file or one file per regest).
...
Input files must be excel files and be in *data/output* folder.
